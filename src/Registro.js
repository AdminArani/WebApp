...
function VerticalLinearStepper2() {
  ...
  const [inputIdentidad, set_inputIdentidad] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  });
  const [inputNombre1, set_inputNombre1] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  ...
  const [inputNombre3, set_inputNombre3] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  ...
  const [inputCorreo, set_inputCorreo] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  ...
  const [inputFechaNac, set_inputFechaNac] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  });
  const [inputTelefono, set_inputTelefono] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  });
  const [inputTelefonoCode, set_inputTelefonoCode] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  ...
  const [inputPass2, set_inputPass2] = useState({
    valor: '',
    validado: false,
    textoAyuda: "",
    blur: false
  ...
  // Agregar el estado para manejar el mensaje de error de identidad
  const [mensajeErrorIdentidad, setMensajeErrorIdentidad] = useState('');
  const [mensajeErrorNombre1, setMensajeErrorNombre1] = useState('');
  const [mensajeErrorNombre3, setMensajeErrorNombre3] = useState('');
  const [datosCenso, setDatosCenso] = useState(null);
  const handleNext = () => {
  ...
  function handleChange_inputIdentidad(event) {
    ...
    if (diffcaracteres === 0) {
      ...
      // Llamar al endpoint
      const url = `${config.apiUrl}/api/app/validardatoscenso.php?numero_identidad=${valor}&apikey=F1A672ACF37354D0EEAAB4F6574729AF`;
      fetch(url).then(response => response.json()).then(data => {
        if (data.status === "success") {
          if (data.data.EDAD < 21) {
            setMensajeErrorIdentidad("Debes ser mayor de 21 años para continuar.");
            setDatosCenso(null);
            validado = false;
          } else {
            setMensajeErrorIdentidad('');
            setDatosCenso(data.data); // Guardamos los datos recibidos
          }
        } else if (data.status === "not_found") {
          setDatosCenso(null);
          setMensajeErrorIdentidad("Por favor, ingrese un número de identidad válido.");
        } else {
          setDatosCenso(null);
          setMensajeErrorIdentidad("Ocurrió un error al validar la identidad.");
        }
      }).catch(error => {
        console.error("Error al consultar validarDatosCenso:", error);
        setMensajeErrorIdentidad("Error de conexión. Intente más tarde.");
      });
    }
    if (diffcaracteres > 0) {
  ...
  function handleChange_inputNombre1(event) {
    ...
    let valor = event.target.value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "") // Elimina todos los espacios
    .toUpperCase();
    let validado = false;
    let textoAyuda = "Solo letras sin espacios";
    if (event.target.value.includes(" ")) {
      textoAyuda = "No se permiten espacios";
    } else if (valor.length >= 1 && /^[A-Z]+$/.test(valor)) {
      // Validar solo letras en mayúsculas sin acentos
      if (datosCenso && datosCenso.DES_PRIMER_NOMBRE) {
        const nombreCenso = datosCenso.DES_PRIMER_NOMBRE.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  ...
  function handleChange_inputNombre2(event) {
    ...
    const valorSinAcentos = event.target.value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  ...
  function handleChange_inputNombre3(event) {
    ...
    if (event.target.value.includes(" ")) {
      ...
      if (datosCenso && datosCenso.DES_PRIMER_APELLIDO) {
        const apellidoCenso = datosCenso.DES_PRIMER_APELLIDO.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase();
  ...
  function handleChange_inputCorreo(event) {
    let valor = event.target.value.toLowerCase(); // Convertir a minúsculas
    let validado = false;
    let textoAyuda = "Ingrese un correo válido";

    // Contador la cantidad de arrobas
    const atCount = (valor.match(/@/g) || []).length;

    // Aquí se corrige la expresión que regula la expresión "usuario@dominio.com"
    let expreg = /^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.[a-z]{2,}$/i;

    // Aquí se valida que solo haya una arroba y que el formato sea correcto
    if (atCount === 1 && expreg.test(valor)) {
      validado = true;
      textoAyuda = "";
    } else if (atCount > 1) {
  ...
  function handleChange_inputFechaNac(nuevoValor) {
    ...
    let validado = false;
    const fechaIngresada = moment(nuevoValor?.$d).format('YYYY-MM-DD');
    if (fechaIngresada && datosCenso && datosCenso.FECHA_NACIMIENTO) {
      const fechaCenso = moment(datosCenso.FECHA_NACIMIENTO).format('YYYY-MM-DD');
      if (fechaIngresada === fechaCenso) {
        validado = true;
        textoAyuda = '';
      } else {
        textoAyuda = 'La fecha no coincide con el DNI ingresado.';
      }
    }
    set_inputFechaNac({
      valor: valor,
  ...
  useEffect(() => {
    ...
    if (Object.keys(dataUsuarioRDB).length) {
      ...
      if (nombreDB !== nombreForm) {
        errorData = true;
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"1"} variant="body2">- El primer nombre no es correcto.</Typography>);
      }
      ...
      if (nombre2DB !== nombre2Form) {
        ...
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"2"} variant="body2">- El segundo nombre no es correcto.</Typography>);
      }
      ...
      if (apellidoDB !== apellidoForm) {
        ...
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"3"} variant="body2">- El primer apellido no es correcto.</Typography>);
      }
      ...
      if (apellido2DB !== apellido2Form) {
        ...
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"4"} variant="body2">- El segundo apellido no es correcto.</Typography>);
      }
      ...
      if (sexoDB !== sexoForm) {
        ...
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"5"} variant="body2">- El sexo seleccionado no es correcto.</Typography>);
      }
      ...
      if (fechanacDB !== fechanacForm) {
        ...
        errorArrayElements.push(<Typography sx={{
          color: 'red'
        }} key={"6"} variant="body2">- La fecha de nacimiento no es correcta.</Typography>);
      }
  ...
  return <Box>
      ...
      mb: 3
            ...
            <Stepper activeStep={activeStep} orientation="vertical">
              ...
              mb: 1
                                ...
                                <Grid item xs={12} sm={12}>
                                    <TextField required autoFocus={!inputIdentidad.valor} autoComplete="off" fullWidth label="Número de identidad" onBlur={() => {
                  set_inputIdentidad({
                    ...inputIdentidad,
                    blur: true
                  });
                }} value={inputIdentidad.valor} onChange={handleChange_inputIdentidad} error={!inputIdentidad.validado && inputIdentidad.blur} helperText={inputIdentidad.textoAyuda} />
                  
                                    {mensajeErrorIdentidad && <p style={{
                  color: 'red',
                  marginTop: '5px'
                }}>{mensajeErrorIdentidad}</p>}
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField required autoComplete="off" fullWidth label="Primer nombre" onBlur={() => {
                  set_inputNombre1({
                    ...inputNombre1,
                    blur: true
                  });
                }} value={inputNombre1.valor} onChange={handleChange_inputNombre1} error={!inputNombre1.validado && inputNombre1.blur} helperText={inputNombre1.textoAyuda} style={{
                ...
                onKeyDown={e => {
                  if (e.key === " ") {
                    e.preventDefault();
                                    ...
                                    {mensajeErrorNombre1 && <p style={{
                  color: 'red',
                  marginTop: '5px'
                }}>{mensajeErrorIdentidad}</p>}
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField autoComplete="off" fullWidth label="Segundo nombre" value={inputNombre2} onChange={handleChange_inputNombre2} helperText="" />
                  
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField autoComplete="off" fullWidth label="Primer apellido" id="primerapellidoreg" required onBlur={() => {
                  set_inputNombre3({
                    ...inputNombre3,
                    blur: true
                  });
                }} value={inputNombre3.valor} onChange={handleChange_inputNombre3} error={!inputNombre3.validado && inputNombre3.blur} helperText={inputNombre3.textoAyuda} style={{
                  textTransform: "uppercase"
                }} // Forzar mayúsculas en la visualización
                onKeyDown={e => {
                  if (e.key === " ") {
                    e.preventDefault();
                                    ...
                                    {mensajeErrorNombre3 && <p style={{
                  color: 'red',
                  marginTop: '5px'
                }}>{mensajeErrorNombre3}</p>}
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField autoComplete="off" fullWidth label="Segundo apellido" value={inputNombre4} onChange={handleChange_inputNombre4} helperText="" />
                  
                                </Grid>
                                
                                <Grid item xs={12} sm={12}>
                                    <Button disabled={pasoValidacionGrupo0 ? false : true} variant="contained" onClick={handleNext} sx={{
                  mt: 1,
                  mr: 1
            ...
            mb: 1
                            ...
                            <Grid item xs={12} sm={12}>
                                <TextField autoComplete="off" fullWidth label="Correo electrónico" required onBlur={() => {
                set_inputCorreo({
                  ...inputCorreo,
                  blur: true
                });
                ...
                }} required error={!inputGenero.validado && inputGenero.blur}>Genero</InputLabel>
                                    <Select onBlur={() => {
                  set_inputGenero({
                    ...inputGenero,
                    blur: true
                  });
                }} required value={inputGenero.valor} onChange={handleChange_inputGenero} label="Genero" error={!inputGenero.validado && inputGenero.blur}>
                            ...
                            <Grid item xs={12} sm={6}>
                                <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={"es"}>
                                    <DatePicker fullWidth openTo="year" maxDate={moment().subtract(21, 'years')._d} minDate={moment().subtract(80, 'years')._d} label="Nacimiento" value={inputFechaNac.valor} onChange={handleChange_inputFechaNac} renderInput={params => <TextField {...params} onBlur={() => {
                  set_inputFechaNac({
                    ...inputFechaNac,
                    blur: true
                  });
                }} autoComplete="off" fullWidth required helperText={inputFechaNac.textoAyuda} error={!inputFechaNac.validado && inputFechaNac.blur} />} />
                ...
                marginTop: '5px'
                            ...
                            <Grid item xs={12} sm={6}>
                                <TextField fullWidth autoComplete="off" label="Teléfono móvil" required onBlur={() => {
                set_inputTelefono({
                  ...inputTelefono,
                  blur: true
                });
              }} disabled={!inputFechaNac.validado || telefonoValidadoPorSMS || smsEnEspera} // <-- Aquí el cambio
              value={inputTelefono.valor} onChange={handleChange_inputTelefono} error={!inputTelefono.validado && inputTelefono.blur} helperText={inputTelefono.textoAyuda} />
                            ...
                            <Grid item xs={12} sm={6}>
                                <Tooltip placement="top" title="Te enviaremos un mensaje de texto a tu teléfono celular con un código para que valides que realmente eres tú el dueño de ese teléfono. Ingresa ese código en el siguiente campo de texto llamado 'Código SMS'.">
                                    <span>
                                        <Button fullWidth disabled={!inputTelefono.validado || enviandoSMSTel || smsEnEspera || telefonoValidadoPorSMS} variant="contained" onClick={validarTel} sx={{
                    mt: 1,
                    mr: 1
                  }}>{enviandoSMSTel || smsEnEspera || telefonoValidadoPorSMS ? "Enviado" : "Enviar sms"}</Button>
                                    </span>
                                </Tooltip>
                                {smsEnEspera && !telefonoValidadoPorSMS && <Typography variant="body2" sx={{
                textAlign: "center",
                color: 'silver'
              }}>Reenviar ({contadorReenvioSMS}s)</Typography>}
                            </Grid>
                            <Grid item xs={12} sm={6}>
                            ...
                            <Grid item xs={12} sm={6}>
                                <Button fullWidth variant="contained" disabled={!inputTelefonoCode.validado || telefonoValidadoPorSMS || !smsEnEspera} onClick={fnValidarCodigoSMS} sx={{
                mt: 1,
                mb: 1
              }}>{telefonoValidadoPorSMS ? "Validado" : "Confirmar"}</Button>
                            </Grid>
                            <Grid item xs={12} sm={12}>
                                <Button disabled={pasoValidacionGrupo1 ? false : true} variant="contained" onClick={handleNext} sx={{
                mt: 1,
                mr: 1
              }}>Paso siguiente</Button>
                                <Button disabled={false} onClick={handleBack} sx={{
                mt: 1,
                mr: 1
            ...
            mb: 1
                            ...
                            <Grid item xs={12} sm={12}>
                                <FormControl fullWidth variant="outlined">
                                    ...
                                    <OutlinedInput required autoComplete="off" fullWidth id="inputPass1" type={inputPassVisible ? 'text' : 'password'} value={inputPass1.valor} onChange={handleChange_inputPass1} onBlur={() => {
                  set_inputPass1({
                    ...inputPass1,
                    blur: true
                  });
                }} label="Contraseña" error={!inputPass1.validado && inputPass1.blur} endAdornment={<InputAdornment position="end">
                                                <IconButton onClick={handleClick_mostrarpass} onMouseDown={e => {
                    e.preventDefault();
                  }} edge="end">
                            ...
                            <Grid item xs={12} sm={12}>
                                <FormControl fullWidth variant="outlined">
                                    ...
                                    <OutlinedInput required autoComplete="off" fullWidth id="inputPass2" onBlur={() => {
                  set_inputPass2({
                    ...inputPass2,
                    blur: true
                  });
                }} type={inputPassVisible ? 'text' : 'password'} value={inputPass2.valor} onChange={handleChange_inputPass2} label="Repetir contraseña" error={!inputPass2.validado && inputPass2.blur} endAdornment={<InputAdornment position="end">
                                                <IconButton onClick={handleClick_mostrarpass} onMouseDown={e => {
                    e.preventDefault();
                  }} edge="end">
                                                    {inputPassVisible ? <span className="material-symbols-outlined">visibility_off</span> : <span className="material-symbols-outlined">visibility</span>}
                            ...
                            <Grid item xs={12} sm={12}>
                                ...
                                <Dialog open={ventanaCondicionesOpen} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                                    ...
                                    <DialogActions>
                                    <Button onClick={() => {
                    set_ventanaCondicionesOpen(false);
                    set_inputAceptoCondiciones(false);
                  }}>No acepto</Button>
                                    <Button autoFocus onClick={() => {
                    set_ventanaCondicionesOpen(false);
                    set_inputAceptoCondiciones(true);
                  }}>Acepto</Button>
                                    </DialogActions>
                                </Dialog>
                            ...
                            {/* {pasoValidacionGrupo2 && (
                             ...
                             Has terminado los primeros pasos para poder crear tu perfil. Presiona el botón <strong>PASAR A ETAPA 2</strong> para continuar con el proceso de registro.<br/>
                                ...
                                <Button variant="contained" disabled={!pasoValidacionGrupo2} onClick={handleNext} sx={{
                mt: 1,
                mr: 1
              }}>Siguiente</Button>
                                <Button disabled={false} onClick={handleBack} sx={{
    ...
    aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description" style={{
      ...
      alignItems: 'center'
        ...
        alignItems: 'center',
        fontWeight: 'bold'
      }}>{"¡Tienes un préstamo pre aprobado!"}</DialogTitle>
                <DialogContent>
        ...
        justifyContent: 'center'
                    ...
                    <Button className="miDialogoBoton" onClick={handleClose} color="primary" autoFocus style={{
          background: 'white',
          alignContent: 'center',
          fontSize: '12px',
          borderRadius: '20px',
      ...
      mb: 1
            ...
            maxWidth: '30rem'
              ...
              handleReset();
                        ...
                        {mensajeFinalError && <Typography variant="body2" sx={{
          color: 'red',
          pt: 2,
          pb: 1
        }}><b>Error: </b>{mensajeFinalError}</Typography>}
                        {mensajeFinalOk && <Typography variant="body2" sx={{
          color: 'green',
          pt: 2,
          pb: 1
        }}>
            ...
            lineHeight: '1'
                    ...
                    {mensajeFinalOk && <Grid item xs={12} sm={12}><Button component={Link} to="/login" fullWidth variant="contained" sx={{
          mt: 1,
          mr: 1
        }}>Volver al formulario de ingreso</Button></Grid>}
                    {!mensajeFinalOk && <>

                            {/* Boton de enviar registro */}
                            <Grid item xs={12} sm={6}>
                                <Button fullWidth disabled={enviandoDataFinal} onClick={enviarARegistroApi} variant="contained" sx={{
            mt: 1,
            mr: 1
          }}>
                            ...
                            <Grid item xs={12} sm={6}><Button fullWidth variant="text" onClick={handleReset} sx={{
            mt: 1,
            mr: 1
          }}>Reiniciar</Button></Grid>
                        </>}
                </Grid>}
        </Box>;
}
      ...
      p: '4px'
        ...
        pb: 5
          ...
          borderRadius: "0.5rem 0.5rem 0 0"
                        ...
                        <Toolbar>
                            <IconButton component={Link} to="/login" edge="start" color="inherit" aria-label="menu" sx={{
              mr: 2
            }}>
