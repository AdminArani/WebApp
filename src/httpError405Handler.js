...
function extractUserContextFromPayload(payload) {
    ...
    const customerId =
        ...
        payload.client_id ||
        payload.clientId ||
        deepFindByKeys(payload, ["customer_id", "customerId", "idCliente", "client_id", "clientId"]);

    const email =
        payload.email ||
        payload.correoElectronico ||
        deepFindByKeys(payload, ["email", "correoElectronico"]);
    ...
    const mobPhone =
        ...
        payload.telefono ||
        payload.celular ||
        deepFindByKeys(payload, ["mob_phone", "phone", "telefono", "celular"]);

    const personCode =
        payload.person_code ||
        payload.identidadCliente ||
        deepFindByKeys(payload, ["person_code", "identidadCliente"]);

    const activeCreditsAmountSum =
        payload.active_credits_amount_sum ||
        payload.activeCreditsAmountSum ||
        deepFindByKeys(payload, ["active_credits_amount_sum", "activeCreditsAmountSum"]);

    const erroresPerfil =
        payload.errores_perfil ||
        payload.erroresPerfil ||
        deepFindByKeys(payload, ["errores_perfil", "erroresPerfil"]);

    const status =
        payload.status ||
        deepFindByKeys(payload, ["status"]);
...
function getNormalizedUserContext() {
    ...
    const customerId =
        currentUserContext.customer_id ||
        currentUserContext.customerId ||
        currentUserContext.idCliente ||
        currentUserContext.client_id ||
        currentUserContext.clientId;

    const firstName = currentUserContext.realname || currentUserContext.nombre || currentUserContext.name;
    const nameParts = [
        firstName,
        currentUserContext.midname,
        currentUserContext.midname2,
        currentUserContext.surname,
    ].map(safeString).filter(Boolean);

    const activeCreditsAmountSum = safeString(
        currentUserContext.active_credits_amount_sum || currentUserContext.activeCreditsAmountSum
    ) || "0";

    const erroresPerfil = safeString(
        currentUserContext.errores_perfil || currentUserContext.erroresPerfil
    ) || "0";
...
function buildMailToLink(supportEmail, statusCode, requestUrl) {
    ...
    const bodyLines = [
        "Hola equipo,",
        "",
        `Estoy reportando un error ${statusCode} en la app.`,
        "Adjunto captura de pantalla.",
        "",
        `Fecha y hora: ${new Date().toISOString()}`,
        `Ruta actual: ${window.location.href}`,
        `Endpoint: ${requestUrl}`,
...
function showSupportModal({ statusCode, requestUrl }) {
    return new Promise((resolve) => {
        ...
        root.render(
            <Dialog
                ...
                maxWidth="sm"
                fullWidth
                aria-labelledby="http-error-dialog-title"
            >
...
async function showErrorFlow({ supportEmail, statusCode, requestUrl }) {
    const now = Date.now();
    ...
    try {
        const shouldOpenEmail = await showSupportModal({ statusCode, requestUrl });
...
export function installHttp405Handler({ supportEmail }) {
    if (isInstalled) {
        return;
    }
    ...
    if (process.env.NODE_ENV !== "production") {
        window.__araniTestHttpErrorModal = (statusCode = 401, requestUrl = "prueba/manual") => {
            void showErrorFlow({ supportEmail, statusCode, requestUrl });
        };
    }
}

export function registerUserContextForErrors({ customerId, email, phone, name }) {
    currentUserContext = {
        ...(currentUserContext || {}),
