...
import { Button, Checkbox, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, MenuItem, Paper, Select, Typography } from "@mui/material";
...
function Aplicar2() {
  ...
  function validarSiTienePrestamos() {
    ...
    axios.request({
      ...
      if (res.data.status === "ERS") {
        ...
        if (typeof set_logeado === "function") set_logeado({
          estado: false,
          token: ""
        });
      }
  ...
  const formatL = amount => {
    ...
    return `L. ${safe.toLocaleString("en-US", {
  ...
  const normalizeOfferDr = apiBody => {
    ...
    const candidates = [apiBody?.offer, apiBody?.data, apiBody?.payload?.offer, apiBody?.payload?.data, parsedPayloadApi?.data, parsedPayloadApi, apiBody?.payload, apiBody].filter(item => item && typeof item === "object");
    const hintKeys = ["outLoanAmount1M", "output1M", "outInterest1M", "outadminFee1M", "interest_anual", "interestAnual", "interes_anual", "interesAnual"];
  ...
  const aplicarVariablesDocumento = rawHtml => {
    ...
    const monthName = String(now.toLocaleString("es-ES", {
      month: "long"
    }) ?? "").toUpperCase();
    ...
    const interestAnualFromOffer = parsePercent(getOfferValue(offerDr, ["interest_anual", "interestAnual", "interes_anual", "interesAnual"]));
    const interestMensualFromOffer = parseMoney(getOfferValue(offerDr, [`outInterest${Number(period) || 0}M`]));
  ...
  const abrirCondiciones = () => {
    ...
    Promise.all([contratoRaw ? Promise.resolve() : cargarContratoTemplate(), pagareRaw ? Promise.resolve() : cargarPagareTemplate()]).finally(() => {
  ...
  useEffect(() => {
    ...
    axios.request({
      ...
      const mensual = products.find(p => isAllowed(p?.ProCod) && String(p?.ProTip ?? "").toLowerCase() === "mensual") || products.find(p => isAllowed(p?.ProCod)) || null;
      if (mensual?.ProCod) {
  ...
  const customerId = usuarioDetalle?.customer_id ?? usuarioDetalle?.customerId ?? usuarioDetalle?.client_id ?? usuarioDetalle?.clientId ?? null;
  useEffect(() => {
    if (!customerId) return;
    set_loadingOffer(true);
    axios.get("https://app.aranih.com/api/DecisionRules/getOfferDrClient.php", {
      params: {
  ...
  const cuotaAprobada = parseMoney(offerDr?.cuota ?? offerDr?.data?.cuota ?? offerDr?.payload?.cuota ?? offerDr?.CUOTA) ?? 0;
  ...
  const fetchFacturaOnce = containerId => {
    ...
    return axios.get(`${config.apiUrl}/api/app/getFacturaV2.php`, {
  ...
  const startFacturaPolling = containerId => {
    ...
    const tick = async () => {
      ...
      if (data) {
        ...
        const total = parseMoney(factura?.totalAPagar ?? factura?.total_a_pagar ?? factura?.total ?? factura?.totalPago ?? factura?.total_pago);
  ...
  const handleRecibir = async () => {
    ...
    try {
      ...
      const extractId = obj => {
        ...
        return obj.container_id ?? obj.containerId ?? obj.loan_offer_id ?? obj.loanOfferId ?? obj.loan_offer?.container_id ?? obj.loan_offer?.containerId ?? obj.loan_offer?.id ?? null;
      };
      ...
      const maybeParsedApi = typeof resBody?.payload?.api === "string" ? safeJsonParse(resBody.payload.api) : null;
      ...
      const containerId = normalizeId(extractId(resBody)) ?? normalizeId(extractId(resBody?.data)) ?? normalizeId(extractId(maybeParsedData)) ?? normalizeId(extractId(resBody?.payload)) ?? normalizeId(extractId(maybeParsedPayload)) ?? normalizeId(extractId(maybeParsedApi)) ??
      // 👈 FIX
      normalizeId(extractId(maybeParsedApi?.data)) ??
      // 👈 FIX DIRECTO
      normalizeId(extractId(resBody?.payload?.data)) ?? normalizeId(extractId(resBody?.data?.loan_offer)) ?? normalizeId(extractId(resBody?.payload?.loan_offer)) ?? normalizeId(extractId(resBody?.payload?.data?.loan_offer)) ?? deepFindContainerId(resBody) ?? deepFindContainerId(maybeParsedData) ?? deepFindContainerId(maybeParsedApi) ??
      // 👈 FIX
      null;
      if (!containerId) {
        ...
        p: {
          ...
          mx: "auto"
            ...
            m: '1rem 0'
                        ...
                        </Box> : tieneOtroPrestamo ? <Box sx={{
          mt: 4,
          mb: 4,
          maxWidth: 520,
          mx: "auto"
        }}>
                                {statusPrestamo === 0 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
                                ...
                                {statusPrestamo === 1 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
                                ...
                                {statusPrestamo === 5 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}>Le informamos que su préstamo ha sido asignado a uno de nuestros agentes y actualmente se encuentra en proceso de validación. Nos esforzamos por asegurarnos de que cada préstamo sea revisado cuidadosamente para garantizar la mejor experiencia de préstamo posible.</Typography>}
                                {statusPrestamo === 4 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}><b>Detalle:</b> Su préstamo ya ha sido pagado por completo.</Typography>}
                                {statusPrestamo === 2 && <Typography variant="body2" sx={{
            color: 'red',
            paddingTop: '1rem'
                                ...
                                {statusPrestamo === 3 && <Typography variant="body2" sx={{
            color: orange[600],
            paddingTop: '1rem'
          }}>Ya tiene un préstamo activo.</Typography>}
                            <Divider sx={{
            m: '1rem 0'
          }} />
                            <Button component={Link} to="/historial" variant="contained" sx={{
            mr: 1
          }}>Historial</Button>
                            <Button component={Link} to="/" variant="outlined">Volver al inicio</Button>
                        </Box> : <>
            ...
            gap: {
              ...
              sm: 2
                        ...
                        {plansUi.map(plan => <Box key={plan.meses} sx={{
              width: "100%",
              maxWidth: 520,
              mx: "auto",
            ...
            p: {
              ...
              alignItems: "center"
                            ...
                            <Typography sx={{
                fontWeight: 700
              }}>{formatL(selectedPlan?.recibes)}</Typography>
                            <Box />
                        </Box>
              ...
              mt: 1
                ...
                fontWeight: 700
                                ...
                                {selectedPlan ? `${formatL(selectedPlan.cuotaMensual)} X ${selectedPlan.meses}` : "L 0.00 X 0"}
                            </Typography>
                            <Box />
                        </Box>
            ...
            p: {
              ...
              sm: 3
                ...
                fontWeight: 700,
                mb: 1
              }}>Proposito del préstamo</Typography>
                            <FormControl fullWidth>
                                <Select value={defaultPurposeId ?? ""} onChange={e => set_defaultPurposeId(String(e.target.value))}>
                                    {Object.keys(purposeListaObj).map(key => <MenuItem key={key} value={String(purposeListaObj[key].id)}>
            ...
            p: {
              ...
              gap: 2
                  ...
                  mt: 0.25
                            ...
                            <Button variant="text" disabled={!hasPlanSelected} onClick={() => {
                if (!hasPlanSelected) return;
                setOpenCambiarCuenta(true);
              }} sx={{
                color: hasPlanSelected ? "#5b75e7" : "text.disabled",
                minWidth: "auto",
                p: 0
              }}>
                                Cambiar
              ...
              alignItems: "center"
                            ...
                            <Checkbox checked={verCondiciones} disabled={!hasPlanSelected} onChange={e => {
                if (!hasPlanSelected) return;
                if (e.target.checked) {
                  abrirCondiciones();
                } else {
                ...
                abrirCondiciones();
                    ...
                    <Dialog open={openContrato} onClose={() => set_openContrato(false)} fullWidth maxWidth="md">
                        ...
                        <DialogContent>
                            {loadingCondiciones && !contratoRaw ? <Box sx={{
                display: "flex",
                justifyContent: "center",
                py: 4
              }}>
                  ...
                  color: "#5b75e7"
                                ...
                                </Box> : <Box>{parse(contratoFinal || "<br/><br/>No hay contrato disponible<br/><br/>")}</Box>}
                        </DialogContent>
                        <DialogActions sx={{
              justifyContent: "flex-start"
                ...
                set_openPagare(true);
                    ...
                    <Dialog open={openPagare} onClose={() => set_openPagare(false)} fullWidth maxWidth="md">
                        ...
                        <DialogContent>
                            {loadingCondiciones && !pagareRaw ? <Box sx={{
                display: "flex",
                justifyContent: "center",
                py: 4
              }}>
                  ...
                  color: "#5b75e7"
                                ...
                                </Box> : <Box>{parse(pagareFinal || "<br/><br/>No hay pagaré disponible<br/><br/>")}</Box>}
                        </DialogContent>
                        <DialogActions sx={{
              justifyContent: "flex-start"
            }}>
                ...
                setVerCondiciones(true);
                    ...
                    <Dialog open={openCambiarCuenta} onClose={() => setOpenCambiarCuenta(false)} fullWidth maxWidth="sm">
                        <DialogContent>
                            {apiCamposConstructor?.bank?.values ? <FormCambiarBanco cerrar={() => setOpenCambiarCuenta(false)} reiniciarpantalla={() => {
                cargarPerfil();
                cargarCuentaBanco();
                setOpenCambiarCuenta(false);
              }} apiCamposConstructor={apiCamposConstructor} usuarioDetalle={usuarioDetalle} /> : <Box sx={{
                display: "flex",
                justifyContent: "center",
                py: 4
              }}>
                  ...
                  color: "#5b75e7"
                    ...
                    <Button fullWidth variant="contained" disabled={!canRecibir || postingOffer} sx={{
            mt: {
              xs: 2,
              sm: 3
            },
            ...
            "&.Mui-disabled": {
              ...
              color: "text.disabled"
                ...
                mr: 1
                    ...
                    <Dialog open={openResumen} onClose={() => {}} // evita cerrar al hacer clic fuera
          ...
          fullWidth maxWidth="sm">
                ...
                py: 3
                  ...
                  color: "#5b75e7"
                            ...
                            {recibirError ? <Typography color="error" sx={{
                mb: 2
              }}>
                                    {recibirError}
                                </Typography> : null}

                            {loadingFactura ? <Box sx={{
                display: "flex",
                justifyContent: "center",
                py: 4
              }}>
                  ...
                  color: "#5b75e7"
                            ...
                            {!loadingFactura && facturaData ? <Box>
                                    <Box sx={{
                  display: "flex",
                  justifyContent: "space-between",
                  mb: 1
                    ...
                    fontWeight: 700
                                        ...
                                        <Typography variant="body2">
                                            {formatL(parseMoney(facturaData?.cantidadSolicitada ?? facturaData?.cantidad_solicitada ?? facturaData?.amount ?? facturaData?.cantidad) ?? 0)}
                                        </Typography>
                                    </Box>
                  ...
                  mb: 1
                    ...
                    fontWeight: 700
                                        ...
                                        <Typography variant="body2">
                                            {formatL(parseMoney(facturaData?.intereses ?? facturaData?.interes ?? facturaData?.interest ?? facturaData?.interest_amount) ?? 0)}
                                        </Typography>
                                    </Box>
                  ...
                  mb: 1
                    ...
                    fontWeight: 700
                                        ...
                                        <Typography variant="body2">
                                            {formatL(parseMoney(facturaData?.gastosAdministrativos ?? facturaData?.gastos_administrativos ?? facturaData?.administrative_fee ?? facturaData?.administrativeFee) ?? 0)}
                                        </Typography>
                                    </Box>
                  ...
                  mb: 1
                    ...
                    fontWeight: 700
                                        ...
                                        <Typography variant="body2">
                                            {formatL(parseMoney(facturaData?.totalAPagar ?? facturaData?.total_a_pagar ?? facturaData?.total ?? facturaData?.totalPago ?? facturaData?.total_pago) ?? 0)}
                                        </Typography>
                                    </Box>

                                    {(() => {
                  const pagosList = Array.isArray(facturaData?.pagos) ? facturaData.pagos : Array.isArray(facturaData?.payments) ? facturaData.payments : Array.isArray(facturaData?.payouts) ? facturaData.payouts : [];
                  ...
                  return <Box sx={{
                    mt: 2
                      ...
                      mb: 1
                                            ...
                                            {pagosList.map((p, idx) => <Box key={idx} sx={{
                      display: "flex",
                      justifyContent: "space-between",
                      mb: 1
                    }}>
