      ...
      width: '100%'
        ...
        p: 4
            ...
            bgcolor: 'background.paper'
              ...
              return <ListItemButton key={key} component={esEstadoBloqueado ? 'div' : Link} // Cambiar a 'div' si está bloqueado
              to={!esEstadoBloqueado ? `/plan/${dataObj[key].container_id}` : undefined} // Solo si no está bloqueado
              className={`listahistorialestado${dataObj[key].status}`} onClick={event => {
                if (esEstadoBloqueado) {
                  event.preventDefault(); // Bloquear clic
                }
              }} sx={{
                cursor: esEstadoBloqueado ? 'not-allowed' : 'pointer',
                // Cambiar cursor
                opacity: esEstadoBloqueado ? 0.6 : 1 // Opacidad más baja si está bloqueado
                  ...
                  pl: 6
                    ...
                    {Object.keys(dataObj).length === 0 && cargando && <Typography variant="body2" sx={{
          p: '4rem 0',
          color: 'silver',
          textAlign: 'center'
        }}>Cargando....</Typography>}
