...
import Pagare from "./componentes/Pagare";
...
function Plan() {
  ...
  useEffect(() => {}, [gContext.logeado?.token]);
  useEffect(() => {
  ...
  const enviarComprobante = clienteData => {
    ...
    setExitoEnvio(false);
    const formData = new FormData();
    const nombreCompleto = [clienteData.realname, clienteData.midname, clienteData.midname2, clienteData.surname].filter(Boolean).join(' ');
  ...
  const handleOpenModal = () => {
    validarPerfilEnCore().then(clienteData => {
  ...
  const calcMontoCuotaN1co = p => {
    ...
    const monto = v(p?.charge) - v(p?.charge_covered) + (v(p?.administrator_fee) - v(p?.administrator_fee_covered)) + (v(p?.amount) - v(p?.amount_covered)) + (v(p?.late_fee) - v(p?.cinterest_covered));
  ...
  const consultarPagoActualN1co = async ({
    idCliente,
  ...
  const validarVentana42hN1co = () => {
    ...
    const createdMoments = listaPagos.map(p => p?.created).filter(Boolean).map(dt => moment(dt));
  ...
  const generarLinkN1co = async () => {
    try {
      ...
      if (!orderCode) {
        throw new Error("No se pudo obtener el código de orden del link generado.");
      }
      setN1coOrderCode(orderCode);

      // INSERTAR REGISTRO PENDIENTE INMEDIATAMENTE
      try {
      ...
      pollingRef.current = setInterval(async () => {
        try {
          ...
          console.error("Error consultando status N1co:", e);
          setN1coPaso("error");
  ...
  const enviarPostNicoPago = async ({
    ...
    const nombreClienteConcat = [clienteData?.realname, clienteData?.midname, clienteData?.midname2, clienteData?.surname].filter(Boolean).join(" ");
    ...
    const comentarioN1co = esFinalizado ? "Pago validado" : "Pago pendiente de validación";
    ...
    const horaRegistro = new Date(new Date().getTime() - 6 * 60 * 60 * 1000).toISOString().split("T")[1].split(".")[0];
    ...
    if (!identificadorPagoCuota || identificadorPagoCuota < 1) {
      throw new Error(`identificadorPago inválido: ${identificadorPagoCuota}. schedule_position=${pagoseleccionado?.schedule_position}`);
    }
    const cuota = calcMontoCuotaN1co(pagoseleccionado);
    const fechaCuota = pagoseleccionado?.schedule_date ? moment(pagoseleccionado.schedule_date).format("YYYY-MM-DD") : null;
    const payload = {
    ...
    const bodyStr = new URLSearchParams(Object.entries(payload).reduce((acc, [k, v]) => {
  ...
  return <Container disableGutters sx={{
    ...
    justifyContent: "center"
            ...
            <Contrato open={ventanaContrato} onClose={() => {
      set_ventanaContrato(false);
    }} />
            <Pagare open={ventanaPagare} onClose={() => {
      set_ventanaPagare(false);
    }} />
            <Box sx={{
      p: '4px',
      width: '100%'
        ...
        p: 4
                    ...
                    {cargando && <Typography variant="body2" sx={{
          p: '8rem 0 8rem 0',
          color: 'silver',
          textAlign: 'center'
        }}>Cargando....</Typography>}
            ...
            mt: 6
                        ...
                        {prestamoSeleccionado.status === 0 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}>Gracias por solicitar un préstamo con ARANI. Queremos informarte que hemos recibido tu solicitud y estamos trabajando en revisar la información que nos has proporcionado.</Typography>}
                        {prestamoSeleccionado.status === 1 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}>Le informamos que su préstamo ha sido asignado a uno de nuestros agentes y actualmente se encuentra en proceso de validación. Nos esforzamos por asegurarnos de que cada préstamo sea revisado cuidadosamente para garantizar la mejor experiencia de préstamo posible.</Typography>}
                        {prestamoSeleccionado.status === 5 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}>Le informamos que su préstamo ha sido asignado a uno de nuestros agentes y actualmente se encuentra en proceso de validación. Nos esforzamos por asegurarnos de que cada préstamo sea revisado cuidadosamente para garantizar la mejor experiencia de préstamo posible.</Typography>}
                        {prestamoSeleccionado.status === 4 && <Typography variant="body2" sx={{
            color: '#26c926',
            paddingTop: '1rem'
          }}><b>Detalle:</b> Su préstamo ya ha sido pagado por completo.</Typography>}
                        {prestamoSeleccionado.status === 2 && <Typography variant="body2" sx={{
            color: 'red',
            paddingTop: '1rem'
          }}>Le informamos que hemos revisado detalladamente la información que nos proporcionó para su solicitud de préstamo en línea, pero lamentablemente, su solicitud ha sido rechazada por uno de nuestros agentes.</Typography>}
              ...
              mb: 2
                  ...
                  verticalAlign: 'middle'
                }}>Deposito por aplicar: <strong>Lps. {numeral(walletData?.amount_wallet_balance).format('0,0.00')}</strong></Typography>
                                </Grid>
                                <br></br>
                            </div>
                            <div className="bloqueprestamopdt-bar">
                                ...
                                <div className="bloqueprestamopdt-barllena" style={{
                width: pagosrealizadosnum / listaPagos.length * 100 + "%"
              }}></div>
                            </div>
                            <br></br>
                            ...
                            <Box className="bloqueprestamopdt-dtll">
                                ...
                                <Chip size="small" label={"Interes " + productoSeleccionado.ProTip + ": " + interesPeriodo + "%"} variant="outlined" sx={{
                color: "#FFFFFF"
              }} />
                                <Chip size="small" label={"Terminos: " + productoSeleccionado.ProTip} variant="outlined" sx={{
                color: "#FFFFFF"
              }} />
                                ...
                                <Chip size="small" label={"Estado: " + nombreEstadoPrestamo[prestamoSeleccionado.status]} variant="outlined" sx={{
                color: "#FFFFFF"
              }} />
                            </Box>
            ...
            mt: 3
              ...
              '&:hover': {
                ...
                boxShadow: 'none'
                        ...
                        {walletData?.amount_wallet_balance >= prestamoSeleccionado.debt ? <div className="pago-completado">
                            <Typography variant="h5" sx={{
              mt: 6,
              textAlign: 'center',
              ...
              fontSize: '1rem'
                                ...
                                <br />
                                <br />
                                <a href="https://www.arani.hn/erroresperfil.php#pagosadelantados" target="_blank" rel="noopener noreferrer" style={{
                color: 'white',
              ...
              mt: 6
                ...
                p: '12px'
                  ...
                  if (!v.ok) {
                    setMsgEsperaN1co(`Aún no puedes realizar tu pago. Debes esperar 42 horas después del desembolso de tu préstamo.\n` + `Te faltan: ${v.faltanTxt}.\n` + `Disponible a partir de: ${v.habilitaEn.locale('es').format("D [de] MMMM [de] YYYY [a las] h:mm A")}`);
              ...
              mt: 2
                            ...
                            {listaPagos && <>
                                {listaPagos.map((element, index) => <ListItem key={element.schedule_date} disablePadding className={"listitempagostatus" + element.status}>
                                    <ListItemText primaryTypographyProps={{
                  component: 'div',
                  fontSize: '1.5rem'
                }} secondaryTypographyProps={{
                  component: 'div'
                }} primary={`Pago ${index + 1}/${listaPagos.length}`} secondary={<>
                                        ...
                                        <Typography variant="h6">
                                            <span>
                                            Lps. {numeral(element.charge - element.charge_covered + element.administrator_fee - element.administrator_fee_covered + element.amount - element.amount_covered + element.late_fee - element.cinterest_covered).format("0,0.[00]")}
                                            </span>
                                        </Typography>
                                        </>} />
                            ...
                            {!listaPagos && <Typography variant="body2" sx={{
              m: '8rem 0',
              textAlign: 'center',
              color: 'silver'
            }} component={"div"}>No tienes lista de pagos</Typography>}
                        </>}

                    </Box>}
                    <Dialog onClose={() => {
          set_openSubirComprobantePago(false);
        }} open={openSubirComprobantePago}>
                    <DialogContent>
                        <Typography variant="h5">Subir archivo TIGO</Typography>
                        <Typography variant="body2" sx={{
              mb: 2
            }}>Adjunte su comprobante de pago Tigo Money.</Typography>
                        ...
                        <Typography variant="body2" sx={{
              mt: 2
            }}>Una vez enviado, será revisado por uno de nuestros agentes para validar que el pago se haya realizado correctamente.</Typography>
                        <Divider sx={{
              mb: 2,
              mt: 2
            }}></Divider>
              ...
              whiteSpace: 'pre-line'
                ...
                {/* Modal para N1co */}
                <Dialog open={openModalN1co} onClose={(event, reason) => {
          ...
          cerrarModalN1co();
              ...
              mt: 1
                ...
                {n1coPaso === "idle" && <Typography variant="body2" sx={{
              mb: 2,
              mt: 1
            }}>
                ...
                {n1coPaso === "esperando" && <Typography variant="body2" sx={{
              mb: 2,
              mt: 1,
              fontWeight: "bold"
            }}>
            ...
            {/* Cuenta regresiva total (10 min) */}
                    {n1coExpiraEnSeg !== null && <Typography variant="body2" sx={{
              mt: 1
            }}>
                        Link expira en:{" "}
                        <b>
                    ...
                    {n1coCierreEnSeg !== null && <Typography variant="body2" sx={{
              mt: 2,
              color: "red",
              fontWeight: "bold"
            }}>
                ...
                {n1coPaso === "pagado" && <Typography variant="body2" sx={{
              mb: 2,
              mt: 1,
              fontWeight: "bold",
              color: "green"
                ...
                <TextField label="Monto a pagar" value={`L. ${n1coAmount}`} fullWidth InputProps={{
              readOnly: true
            }} sx={{
              mt: 1,
              ...
              gap: 1
                    ...
                    <Button variant="contained" onClick={() => setOpenModalAvisoConfirmacionN1co(true)} disabled={cargandoLinkN1co || accionPendienteN1co || !n1coAmount || n1coPaso === "esperando" || n1coPaso === "pagado"}>
                ...
                <Dialog open={openModalReintentoN1co} onClose={() => setOpenModalReintentoN1co(false)}>
              ...
              whiteSpace: "pre-line"
                    ...
                    <DialogActions>
                        <Button onClick={() => setOpenModalReintentoN1co(false)} variant="outlined">

                            Cancelar
                        </Button>

                        <Button onClick={confirmarReintentoLinkN1co} variant="contained">
                ...
                <Dialog open={openModalAvisoConfirmacionN1co} onClose={() => {
          ...
          setExitoEnvio(false); // Resetea el mensaje de éxito al cerrar el modal
              ...
              mt: 2
                    ...
                    <TextField label="Número de Referencia" value={numReferencia} onChange={handleNumReferenciaChange} type="text" fullWidth sx={{
              mt: 2
            }} error={!!estadoReferencia} helperText={estadoReferencia} />


                    <Button fullWidth variant="contained" component="label" startIcon={<span className="material-symbols-outlined">cloud_upload</span>} sx={{
              mt: 2
            ...
            {cargandoEnvio && <Typography variant="body2" sx={{
                color: 'blue',
                mt: 2
              }}>
                    Cargando...
            ...
            <Modal open={openConfirmacionPago} onClose={() => setOpenConfirmacionPago(false)} aria-labelledby="confirmacion-pago-title" aria-describedby="confirmacion-pago-description">
                ...
                <Box sx={{
                  ...
                  zIndex: 1300 // Asegúrate de que esté por encima de otros elementos
                    ...
                    <img src={`${process.env.PUBLIC_URL}/logowhite.png`} alt="Logo" height={'100px'} width={'100px'} style={{
                    marginBottom: '16px'
                  }} />

                    <Typography id="confirmacion-pago-title" variant="h6" component="h2" sx={{
                    color: 'white',
                    fontWeight: 'bold'
                  }}>
              ...
              width: '100%'
                        ...
                        <Button onClick={() => enviarComprobante(clienteData)} // Usa los datos del cliente obtenidos previamente
              disabled={!fotoComprobante} variant="contained">
                    ...
                    {!cargando && !prestamoSeleccionado && <Box sx={{
          m: '8rem 0',
          textAlign: 'center',
          color: 'silver'
        }}>
            ...
            margin: '0 auto',
            pb: 3
          }} component={"div"}>No tienes préstamos aprobados aún, puedes solicitar un nuevo préstamo o ver el estado de tu solicitud.</Typography>
                        {(() => {
