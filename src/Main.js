  ...
  async function guardarUbicacion(customerId) {
    navigator.geolocation.getCurrentPosition(async position => {
      ...
      try {
        ...
        const localidadCruda = address.city || address.town || address.village || address.municipality || address.county || address.suburb || address.neighbourhood || '';
  ...
  return <Container disableGutters sx={{
    ...
    justifyContent: "center"
      ...
      backgroundColor: '#5b75e7'
                    ...
                    <img src={process.env.PUBLIC_URL + '/araniloader1.gif'} alt="loading" style={{
        height: '250px',
        width: '250px'
      }} />
                  </div> : <Box sx={{
        ...
        p: 6
          ...
          mt: 6
                    ...
                    <p>
                        ...
                        {loading ? 'Cargando...' : <span style={{
            color: ubicacion === 'Para continuar con el proceso de solicitud de préstamo en Arani App, es necesario que el navegador acceda a tu ubicación en tu celular.' ? 'red' : 'black'
          }}>
                                {ubicacion}
                                {ubicacion === 'Para continuar con el proceso de solicitud de préstamo en Arani App, es necesario que el navegador acceda a tu ubicación en tu celular.' && <a href="https://www.arani.hn/erroresperfil.php#ubicacion" target="_blank" rel="noopener noreferrer" style={{
              marginLeft: '8px',
              color: 'blue'
            }}>
                                        Ver más
                    ...
                    {usuarioDetalle.status === "1" && !showAplicarLink && ubicacion !== "Para continuar con el proceso de solicitud de préstamo en Arani App, es necesario que el navegador acceda a tu ubicación en tu celular." && <div style={{
          color: "#e74c3c",
          fontWeight: "bold",
          marginBottom: 16
        }}>
            ...
            if (errorKey !== "20") {
              ...
              if (errorKey === "0") {
                return <div style={styles.containerOk}>
              ...
              return <div style={styles.container}>
                                            <Typography style={styles.text} dangerouslySetInnerHTML={{
            ...
            return <div style={styles.container}>
                ...
                marginTop: '8px'
                                            ...
                                            <a href="https://app.arani.hn/#/perfil" target="_blank" rel="noopener noreferrer" style={{
                  color: '#3498db',
                  textDecoration: 'none'
                }}>
                                                Ir a mi perfil
          ...
          const fechasFeriadoAnuncio = new Set(["2026-06-27", "2026-06-28"]);
          ...
          return null; // No mostrar nada si no está dentro del rango de fechas
                    ...
                    <div className="contetilebotonpri">
                        {["21", "22", "23"].includes(usuarioDetalle?.errores_perfil) ? <Link to="#" className="tilebotonpri disabled" aria-disabled="true" tabIndex={-1} onClick={e => e.preventDefault()}>
                            ...
                            </Link> : <Link to="/perfil" className="tilebotonpri">
                                <div className="tilebotonpri-tit">Perfil</div>
            ...
            const fechasFeriado = new Set(["2026-06-27", "2026-06-28"]);
              ...
              return <Link className="tilebotonpri disabled" style={{
                ...
                wordWrap: 'break-word'
                  ...
                  flex: 1
                                        ...
                                        <div className="tilebotonpri-tit" style={{
                    textAlign: 'left'
                  }}>Aplicar</div>
            ...
            if (usuarioDetalle.status === "0" || usuarioDetalle.status === 0) {
              return <Link className="tilebotonpri disabled" to="#" onClick={e => e.preventDefault()}>
            ...
            if ((usuarioDetalle.status === "1" || usuarioDetalle.status === 1) && !showAplicarLink) {
              return <Link className="tilebotonpri disabled">
                                        <div className="tilebotonpri-tit">Aplicar</div>
                                        <div className="tilebotonpri-desc">
                                            Lo sentimos, actualmente esta función no está permitida en tu ubicación.
                                        </div>
            ...
            // Caso normal
            return showAplicarLink ? <Link to={!usuarioDetalle.ref_tipo_per || !usuarioDetalle.ref_nom_per || !usuarioDetalle.ref_tel_per ? "#" : "/aplicar2"} className="tilebotonpri" onClick={async e => {
              if (!usuarioDetalle.ref_tipo_per || !usuarioDetalle.ref_nom_per || !usuarioDetalle.ref_tel_per || !usuarioDetalle.ref_tipo_lab || !usuarioDetalle.ref_nom_lab || !usuarioDetalle.ref_tel_lab) {
                e.preventDefault();
                setShowModal(true);
              } else {
                ...
                try {
                  ...
                  try {
                    tipoClienteParsed = JSON.parse(tipoClienteRaw);
                  } catch (e) {}

                  // Extraer lastLoanDaysLate de la respuesta
                  const lastLoanDaysLate = tipoClienteParsed && typeof tipoClienteParsed === 'object' ? Number(tipoClienteParsed.lastLoanDaysLate ?? 0) : 0;

                  // Determinar tipo de cliente para la redirección
                  let tipoClienteResult = typeof tipoClienteParsed === 'object' ? String(tipoClienteParsed.tipo ?? tipoClienteParsed).trim() : String(tipoClienteParsed).trim();
                  ...
                  const hoy = new Date();
                  const dias = inicioWork && !Number.isNaN(inicioWork.getTime()) ? Math.max(0, Math.floor((hoy.getTime() - inicioWork.getTime()) / (1000 * 60 * 60 * 24))) : 0;
                  ...
                  setLoading(false);
                                    ...
                                    <div className="tilebotonpri-estado"></div>
                                    <div className="tilebotonpri-icon"><img alt="" src={tile_aplicar} /></div>
                                </Link> : <Link className="tilebotonpri disabled">
                                    <div className="tilebotonpri-tit">Aplicar</div>
                                    <div className="tilebotonpri-desc">
                                        {ubicacion && ubicacion !== "Para continuar con el proceso de solicitud de préstamo en Arani App, es necesario que el navegador acceda a tu ubicación en tu celular." ? `Lo sentimos, esta opción no está disponible en ${ubicacion}` : "Lo sentimos es necesario que el navegador acceda a tu ubicación en tu celular."}
                                    </div>
                                    <div className="tilebotonpri-estado"></div>
                                    <div className="tilebotonpri-icon"><img alt="" src={tile_aplicar} /></div>
                                </Link>;
                    ...
                    <Button onClick={salir} variant="contained" sx={{
          mt: 3
            ...
            <ModalDatosFaltantes open={showModal} onClose={() => setShowModal(false)} onSave={handleSaveDatosFaltantes} />
